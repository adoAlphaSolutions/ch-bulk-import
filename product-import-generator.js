...
// Only these top-level categories are offered in the dropdown.
const ALLOWED_CATEGORY_IDS = new Set([
  'TB.PCM.Category.Tiles',          // Tile
  'TB.PCM.Category.Plumbing',       // Plumbing
  'TB.PCM.Category.InteriorDetails',// Interior Details
  'TB.PCM.Category.Flooring',       // Flooring
  'TB.PCM.Category.SolidSurfaces',  // Solid Surfaces
  'TB.PCM.Category.Appliances'      // Appliances
]);
  ...
  return {
    render(context) {
      ...
      function fillCategories() {
        ...
        const filtered = categories.filter(c => ALLOWED_CATEGORY_IDS.has(c[0]));
        const use = filtered.length ? filtered : categories;
        catSel.innerHTML = use.map(([id, label]) => `<option value="${id}">${label}</option>`).join('');
        catSel.value = use.some(c => c[0] === prev) ? prev : (use.some(c => c[0] === DEFAULT_CATEGORY_ID) ? DEFAULT_CATEGORY_ID : (use[0] && use[0][0]));
        fillSubcategories();
      }
      fillCategories();
